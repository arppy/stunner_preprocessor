...
def toString(record):
  ...
  if "localIP" in record and record["localIP"] is not None :
    locipstring = replaceNullNA(replaceProblematicChars(str(record["localIP"])))
    hashSHA256 = hashlib.sha256()
    hashSHA256.update((locipstring + "-" + locipstring).encode())
    tostring += ';' + str(hashSHA256.hexdigest())  # $12
  else :
    tostring += addNAToString(1)
  if "wifiDTO" in record and record["wifiDTO"] is not None:
    ...
    if "ssid" in record["wifiInfo"] and record["wifiInfo"]["ssid"] is not None :
      tostring += ';' + replaceNullNA(replaceProblematicChars(removeLastCharsFromString(str(record["wifiInfo"]["ssid"]))))  # $14
    else :
      tostring += addNAToString(1)
  ...
  if "natResultsDTO" in record and record["natResultsDTO"] is not None:
    ...
    if "publicIP" in record["natResultsDTO"] and record["natResultsDTO"]["publicIP"] is not None:
      ...
      hashSHA256 = hashlib.sha256()
      hashSHA256.update((ipstring+"-"+ipstring).encode())
      tostring += ';' + str(hashSHA256.hexdigest())  # $27
    else:
      tostring += addNAToString(1)
  ...
  latitudeString = str(record["latitude"])
  hashSHA256 = hashlib.sha256()
  hashSHA256.update((latitudeString + "-" + latitudeString).encode())
  tostring += ';' + str(hashSHA256.hexdigest())  # $47
  longitudeString = str(record["longitude"])
  hashSHA256 = hashlib.sha256()
  hashSHA256.update((longitudeString + "-" + longitudeString).encode())
  tostring += ';' + str(hashSHA256.hexdigest())  # $48
  ...
  for fileName in fileList:
    ...
    try:
      with open('' + INFILE_PATH + fileName) as csvfile:
        ...
        i = 0;
        platform = "hu.uszeged.inf.wlab.stunner"
            ...
            if i >= 2:
              if re.match('^hu', record) is not None:
                ...
                try:
                  ...
                  record = replaceProblematicCharsInJSON(str(record))
                  try:
                    record = json.loads(record, strict=False)
                    if not type(record) == type({}):
                      raise ValueError
                ...
                if record:
                  allRecord += 1
                  if "androidID" in record and record["androidID"] is not None and record["androidID"] != "":
                    userName = record["androidID"]
                  record = determineUnkownValue(record)
                  record["sourceFile"] = fileName
                  record["sourceRow"] = sourceRow(baseNameString[0],counterPerFile)
                  ...
                  try:
                    ...
                    try:
                      outFileName = '' + OUTFILE_PATH + str(outSufix) + '/' + userName
                      file = open(outFileName, "a+", encoding="utf-8")
                      file.write('' + outstr + '\n')
                      file.close()
                    except Exception as e:
                      print('File write error ', outFileName, str(userName), str(appVersion), str(line), type(e), e, e.args)
                  except Exception as e:
                    allFiltered += 1
...
def sourceRow(fileCreationDate,counterPerFile) :
  outStr=''+str(fileCreationDate)
  prefix=''+str(counterPerFile)
  for i in range(len(prefix),9) :
    prefix='0'+prefix
  return outStr+prefix
