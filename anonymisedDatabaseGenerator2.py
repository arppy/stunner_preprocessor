...
#            data = list(reader)
#            count1 += len(data)
    if re.match(r'^[s-zS-Z]', filename):
        startWithTtoZ.append(filename)

for filename in os.listdir('D:/info2/tdk/Stunner_meresek/user_preproceed_v2/out4'):
#    with open('D:/info2/tdk/Stunner_meresek/user_preproceed_v2/out4/' + filename, encoding='utf8') as csvfile:
#            reader = csv.reader(csvfile, delimiter=';')
#            data = list(reader)
#            count1 += len(data)
    v2Files.append(filename)
print("step1")

#for filename in os.listdir('D:/info2/tdk/Stunner_meresek/stunner/data'):
#    with open('D:/info2/tdk/Stunner_meresek/stunner/data/' + filename, encoding='utf8') as csvfile:
#            reader = csv.reader(csvfile, delimiter=';')
#            data = list(reader)
#            count2 += len(data)
#
            ...
            for row in reader:
                ...
                try:
                    ...
                    if tempDate in tempDict:
                        tempDict[tempDate] = tempDict[tempDate].append(row)
                    else:
                        tempList = []
                        tempList.append(row)
...
def readAndSortCSV1(myArray):
    headers = ["fileCreationDate", "serverSideUploadDate", "androidID", "recordID", "timeStamp", "timeZoneUTCOffset", "triggerCode", "androidVersion", "appVersion", "connectionMode", "networkInfo", "localIP", "macAddress", "ssid", "state", "bandwidth", "rssi", "carrier", "networkType", "networkCountryIso", "simCountryIso", "roaming", "phoneType", "airplane", "discoveryResult", "exitStatus", "publicIP", "STUNserver", "lastDiscovery", "connectionStart", "connectionEnd", "channelOpen", "channelClosed", "exitStatus", "lastDisconnect", "chargingState", "pluggedState", "percentage", "health", "present", "technology", "temperature", "voltage", "turnOnStimestamp", "shutdownTimestamp", "uptime", "latitude", "longitude", "locationCaptureTimestamp", "country", "system_organization", "continent"]
    tempDict = {}
    for filename in myArray:
        with open('D:/info2/tdk/Stunner_meresek/user_preproceed_v2/out4/' + filename, encoding='utf8') as csvfile:
            reader = csv.reader(csvfile, delimiter=';')
        ...
        print("step2")
        for key in tempDict:
            fileEmpty = os.path.exists('D:/info2/tdk/Stunner_meresek/stunner/data/v2/' + key + '.csv')
            with open('D:/info2/tdk/Stunner_meresek/stunner/data/v2/' + key + '.csv', "a", newline='', encoding='utf-8') as csv_file:
                writer = csv.writer(csv_file, delimiter=';')
                if not fileEmpty:
                    writer.writerow(headers)
                for value in tempDict[key]:
...
try:
    threads = []
    t5 = threading.Thread(target=readAndSortCSV, args=(startWithTtoZ,))
    t6 = threading.Thread(target=readAndSortCSV1, args=(v2Files,))
    threads.append(t5)
    threads.append(t6)
    t5.start()
    t6.start()
