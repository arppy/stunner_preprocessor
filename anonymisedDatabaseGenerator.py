...
for filename in os.listdir('D:/info2/tdk/Stunner_meresek/user_preproceed_v1/out4'):
    if re.match(r'^[0-9]', filename):
        startWithNumber.append(filename)
    elif re.match(r'^[a-fA-F]', filename):
        startWithAtoF.append(filename)
...
try:
    threads = []
    t1 = threading.Thread(target=readAndSortCSV, args=(startWithNumber,))
    t2 = threading.Thread(target=readAndSortCSV, args=(startWithAtoF,))
    threads.append(t1)
    threads.append(t2)
    t1.start()
    t2.start()
